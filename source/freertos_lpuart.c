...
static void  send_data(uint8_t *Data,uint8_t Len) {
	LPUART_RTOS_Send(&handle, Data, Len) ;
}
int main(void)
{
    /* Init board hardware. */
...
static void uart_task(void *pvParameters)
{
    ...
    do
    {
    	...
    	mb_tx_packet_handler(mb_packet_request_read_holding_registers(0x01,0x0000,10));
    	 int status = LPUART_RTOS_Receive(&handle, buffer, 256 , &num);
